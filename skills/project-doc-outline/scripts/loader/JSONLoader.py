...
# -*- coding:utf-8 -*-
"""
JSONLoader 模块（project-doc-query skill 自带）

忠实复制自原版，已改造：
- 替代 langchain_community.JSONLoader 为 Python json 模块
- 支持 jq_schema 简化版（.key.nested 路径访问）
- text_content 控制是否把值当文本
...
class JSONLoader:
    ...
    def _resolve_jq(self, data: Any, schema: str) -> List[Any]:
        """简化版 jq_schema：支持 ".key"、".[].key"、"[]" 这几种常见写法"""
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("用法: python JSONLoader.py <json_path>")
        sys.exit(1)
    loader = JSONLoader(sys.argv[1])
    if loader.exists:
        docs = loader.load()
        print(f"加载 {len(docs)} 个项")
    else:
